#!/bin/bash
###API that remove host from /acpc/adm/etc/badpcs
...
HOST="${GETTYPE}${GETID}"
isBad "${HOST}" 
[ ${?} -eq 0 ] && genError 404 "The host ${HOST} is not listed in badpcs" 7

### Now, replacing the old ip with new ip in the file
TMPFILE=$(mktemp)
sed "/${HOST}/d" ${FILE} > ${TMPFILE}
cp ${TMPFILE} ${FILE}
rm ${TMPFILE}
	initResponse
	startJSON
	...
	insertJSON "response" S "Removed ${HOST}" L
	closeJSON
	printJSON
exit 0
